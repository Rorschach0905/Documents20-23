                                        ...
                                        NApiCore::ICustomPropertyDataApi_1_0* simData)
{
    ...
    if(/*time > 1e-7 ||*/ m_numberCreated >= 2312)
    {
    ...
    // Particle positions
    int c = m_numberCreated / 68;
    int i = m_numberCreated % 68;
    double angle = i * 2 * PI / 68;
    double radius = 0.00095;

    posX = ((-sin(angle))/ 43.55)-0.155;
    posY = ((cos(angle))/ 43.55)-(0.37*(c * 10 * radius + (8 * radius)))+0.173;
    posZ = (c * 10 * radius + (8 * radius))+(((cos(angle))/ 43.55)*0.37)-0.319;
