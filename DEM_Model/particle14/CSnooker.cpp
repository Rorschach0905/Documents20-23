                                        ...
                                        NApiCore::ICustomPropertyDataApi_1_0* simData)
{
    ...
    if(/*time > 1e-7 ||*/ m_numberCreated >= 2720)
    {
    ...
    // Particle positions
    int c = m_numberCreated / 16;
    int i = m_numberCreated % 16;
    double angle = (i * 2 * PI / 16) + (PI / 16);
    double radius = 0.00095;

    posX = ((-sin(angle))/93.7)-0.155;
    posY = ((cos(angle))/93.7)-(0.74 * c * radius)+0.173;
    posZ = (c * 2 * radius)+(((cos(angle))/93.7)*0.37)-0.319;

    strcpy(type, "particle14");
...
void CSnooker::getSmallestScale(double& scale, char type[NApi::API_BASIC_STRING_LENGTH]) const
{
	...
	std::string particleType = "particle14";
