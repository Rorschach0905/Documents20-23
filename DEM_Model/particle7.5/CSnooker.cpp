                                        ...
                                        NApiCore::ICustomPropertyDataApi_1_0* simData)
{
    ...
    if(/*time > 1e-7 ||*/ m_numberCreated >= 884)
    {
    ...
    // Particle positions
    int c = m_numberCreated / 26;
    int i = m_numberCreated % 26;
    double angle = i * 2 * PI / 26;
    double radius = 0.00095;

    posX = ((-sin(angle))/ 116)-0.155;
    posY = ((cos(angle))/ 116)-(0.37*(c * 10 * radius + (8 * radius)))+0.173;
    posZ = (c * 10 * radius + (8 * radius))+(((cos(angle))/ 116)*0.37)-0.319;
