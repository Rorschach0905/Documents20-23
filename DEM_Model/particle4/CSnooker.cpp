                                        ...
                                        NApiCore::ICustomPropertyDataApi_1_0* simData)
{
    ...
    if(/*time > 1e-7 ||*/ m_numberCreated >= 7480)
    {
    ...
    // Particle positions
    int c = m_numberCreated / 44;
    int i = m_numberCreated % 44;
    double angle = (i * 2 * PI / 44) + (PI / 44);
    double radius = 0.00095;

    posX = ((-sin(angle))/34.49)-0.155;
    posY = ((cos(angle))/34.49)-(0.74 * c * radius)+0.173;
    posZ = (c * 2 * radius)+(((cos(angle))/34.49)*0.37)-0.319;

    strcpy(type, "particle4");
...
void CSnooker::getSmallestScale(double& scale, char type[NApi::API_BASIC_STRING_LENGTH]) const
{
	...
	std::string particleType = "particle4";
