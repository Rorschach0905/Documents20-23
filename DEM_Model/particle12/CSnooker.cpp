                                        ...
                                        NApiCore::ICustomPropertyDataApi_1_0* simData)
{
    ...
    if(/*time > 1e-7 ||*/ m_numberCreated >= 3060)
    {
    ...
    // Particle positions
    int c = m_numberCreated / 18;
    int i = m_numberCreated % 18;
    double angle = (i * 2 * PI / 18) + (PI / 18);
    double radius = 0.00095;

    posX = ((-sin(angle))/78.7)-0.155;
    posY = ((cos(angle))/ 78.7)-(0.74 * c * radius)+0.173;
    posZ = (c * 2 * radius)+(((cos(angle))/78.7)*0.37)-0.319;

    strcpy(type, "particle12");
...
void CSnooker::getSmallestScale(double& scale, char type[NApi::API_BASIC_STRING_LENGTH]) const
{
	...
	std::string particleType = "particle12";
