                                        ...
                                        NApiCore::ICustomPropertyDataApi_1_0* simData)
{
    ...
    if(/*time > 1e-7 ||*/ m_numberCreated >= 1088)
    {
    ...
    // Particle positions
    int c = m_numberCreated / 32;
    int i = m_numberCreated % 32;
    double angle = i * 2 * PI / 32;
    double radius = 0.00095;

    posX = ((-sin(angle))/30.303)-0.155;
    posY = ((cos(angle))/30.303)-(0.37 * (c * 10 * radius + (2 * radius)))+0.173;
    posZ = (c * 10 * radius + (2 * radius))+(((cos(angle))/30.303) * 0.37)-0.319;
