                                        ...
                                        NApiCore::ICustomPropertyDataApi_1_0* simData)
{
    ...
    if(/*time > 1e-7 ||*/ m_numberCreated >= 2448)
    {
    ...
    // Particle positions
    int c = m_numberCreated / 72;
    int i = m_numberCreated % 72;
    double angle = i * 2 * PI / 72;
    double radius = 0.00095;

    posX = ((-sin(angle))/ 40.02)-0.155;
    posY = ((cos(angle))/ 40.02)-(0.37 * (c * 10 * radius + (8 * radius)))+0.173;
    posZ = (c * 10 * radius + (8 * radius))+(((cos(angle))/ 40.02)*0.37)-0.319;
