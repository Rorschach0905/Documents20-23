                                        ...
                                        NApiCore::ICustomPropertyDataApi_1_0* simData)
{
    ...
    if(/*time > 1e-7 ||*/ m_numberCreated >= 9180)
    {
    ...
    // Particle positions
    int c = m_numberCreated / 54;
    int i = m_numberCreated % 54;
    double angle = (i * 2 * PI / 54) + (PI / 54);
    double radius = 0.00095;

    posX = ((-sin(angle))/28.57)-0.155;
    posY = ((cos(angle))/28.57)-(0.74 * c * radius)+0.173;
    posZ = (c * 2 * radius)+(((cos(angle))/28.57)*0.37)-0.319;

    strcpy(type, "particle2");
...
void CSnooker::getSmallestScale(double& scale, char type[NApi::API_BASIC_STRING_LENGTH]) const
{
	...
	std::string particleType = "particle2";
