                                        ...
                                        NApiCore::ICustomPropertyDataApi_1_0* simData)
{
    ...
    if(/*time > 1e-7 ||*/ m_numberCreated >= 4760)
    {
    ...
    double angle = (i * 2 * PI / 28) + (PI / 28);
    double radius = 0.00095;

    posX = ((-sin(angle))/ 52.78)-0.155;
    posY = ((cos(angle))/ 52.78)-(0.74 * c * radius)+0.173;
    posZ = (c * 2 * radius)+(((cos(angle))/52.78)*0.37)-0.319;

    strcpy(type, "particle10");
...
void CSnooker::getSmallestScale(double& scale, char type[NApi::API_BASIC_STRING_LENGTH]) const
{
	...
	std::string particleType = "particle10";
