                                        ...
                                        NApiCore::ICustomPropertyDataApi_1_0* simData)
{
    ...
    if(/*time > 1e-7 ||*/ m_numberCreated >= 3400)
    {
    ...
    // Particle positions
    int c = m_numberCreated / 100;
    int i = m_numberCreated % 100;
    double angle = i * 2 * PI / 100;
    double radius = 0.00095;

	posX = ((-sin(angle))/30.303)-0.155;
    posY = ((cos(angle))/30.303)-(0.37 * (c * 10 * radius + (8 * radius)))+0.173;
    posZ = (c * 10 * radius + (8 * radius))+(((cos(angle))/30.303) * 0.37)-0.319;
