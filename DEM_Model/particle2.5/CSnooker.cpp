                                        ...
                                        NApiCore::ICustomPropertyDataApi_1_0* simData)
{
    ...
    if(/*time > 1e-7 ||*/ m_numberCreated >= 3128)
    {
    ...
    // Particle positions
    int c = m_numberCreated / 92;
    int i = m_numberCreated % 92;
    double angle = i * 2 * PI / 92;
    double radius = 0.00095;

    posX = ((-sin(angle))/ 32.258)-0.155;
    posY = ((cos(angle))/ 32.258)-(0.37*(c * 10 * radius + (8 * radius)))+0.173;
    posZ = (c * 10 * radius + (8 * radius))+(((cos(angle))/ 32.258)*0.37)-0.319;
