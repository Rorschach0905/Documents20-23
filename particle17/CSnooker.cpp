                                        ...
                                        NApiCore::ICustomPropertyDataApi_1_0* simData)
{
    ...
    posX = ((-sin(angle))/400)-0.155;
    posY = ((cos(angle))/400)-(0.74 * c *radius)+0.173;
    posZ = (c * 2 * radius)+(((cos(angle))/400)*0.37)-0.319;

    strcpy(type, "particle17");
...
void CSnooker::getSmallestScale(double& scale, char type[NApi::API_BASIC_STRING_LENGTH]) const
{
	...
	std::string particleType = "particle17";
